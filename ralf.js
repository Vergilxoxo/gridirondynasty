...
async function loadRosterPage(rosterId) {
  ...
  function mapPlayer(id) {
    ...
    return {
      ...sleeper,
      ...sheet,
      player_id: sleeper.player_id || sheet["Player ID"] || id,
      full_name: sleeper.full_name || sheet.Name || "-",
      position: sleeper.position || sheet.Position || "-",
      team: sleeper.team || sheet["NFL Team"] || "-"
    };
  }
...
function renderSalaryCapBlock(sheetData, cutSheetData, rosterName, yearCols, activeIds, irIds) {
  ...
  // Wichtig:
  // Spieler, die nicht in der Gehaltstabelle stehen, werden hier herausgefiltert.
  const activePlayers = activeIds
    .map(id => sheetData.find(p => String(p["Player ID"]) === id))
    .filter(Boolean);

  const irPlayers = irIds
    .map(id => sheetData.find(p => String(p["Player ID"]) === id))
    .filter(Boolean);
  ...
  fixedRows.forEach(rowName => {
    ...
    yearCols.forEach(y => {
      ...
      switch (rowName) {
        case "Team Salary Cap":
          value = teamCap;
          break;

        case "Used Salary Cap":
          value = sumForYear(activePlayers, y);
          break;

        case "Injured Reserve":
          value = sumForYear(irPlayers, y);
          break;

        case "Dead Cap":
          value = sumDeadCapForYear(deadCapPlayers, y);
          break;

        case "Team Cap Space":
          value =
            teamCap -
            sumForYear(activePlayers, y) -
            sumDeadCapForYear(deadCapPlayers, y);
          break;
      }
